...
export default function InspectionSlotScreen() {
  ...
  const slotsQuery = useQuery({
    queryKey: [...queryKeys.slots(id, window.from, window.to), 'INSPECTION'],
    queryFn: () => apiClient.listSlots(id, window.from, window.to, 'INSPECTION'),
    enabled: Boolean(id),
  });
  ...
  useEffect(() => {
    const preferred = daySlots.find((slot) => slot.available && slot.label.includes('11:00'));
  ...
  const confirm = useMutation({
    mutationFn: async () => {
      ...
      const hold = await apiClient.createSlotHold(id, selected.slot_id, newIdempotencyKey(`hold-ir-${id}`));
      return apiClient.bookJobCard(id, hold.hold.id, newIdempotencyKey(`book-ir-${id}`), 'INSPECTION');
    },
    onSuccess: (result) => {
      track('inspection_booked', { id });
      router.replace(`/booking/${result.booking.id}?phase=visit1`);
    },
    onError: (err) => {
      const code = err instanceof ApiError ? err.problem?.code : null;
      if (code === 'SLOT_UNAVAILABLE' || code === 'HOLD_EXPIRED') {
        setError('That time was just taken. Pick another slot.');
        void slotsQuery.refetch();
        setSelected(null);
        return;
      }
      setError(err instanceof ApiError ? err.message : 'Could not confirm this inspection slot.');
    },
  });

  return (
    <FlowScreen>
      <InspectionFlowRail currentStep={9} />
      <Text style={styles.title}>Pick inspection slot</Text>
      <Text style={styles.sub}>Visit 1 · Inspection · ~1 hr</Text>
      <PolicyNote>Technician will inspect and document — no repair on this visit.</PolicyNote>
      ...
      <PrimaryButton
        label={selected ? `Confirm inspection · ${selected.label}` : 'Confirm inspection'}
        disabled={!selected}
