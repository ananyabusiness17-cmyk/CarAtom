...
/** Hosts the OSM/MapLibre WebView is allowed to load. Keep in sync with technician MapPreview. */
...
export function buildOsmMapHtml({
  ...
  const color = pinColor.replace(/[^#A-Fa-f0-9]/g, '') || '#E07A3D';
  const custom =
...
<script>
  ...
  const marker = new maplibregl.Marker({ color: '${color}', draggable: ${interactive} })
    .setLngLat([${lng}, ${lat}])
    .addTo(map);
  function emit() {
