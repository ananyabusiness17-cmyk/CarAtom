...
import { technicianRouteForUrl } from '../../src/linking/useDeepLinkHandler';
...
export default function TechnicianNotificationsScreen() {
  ...
  function open(item: Notification) {
    ...
    const href = technicianRouteForUrl(item.deep_link_path || item.deep_link || '');
  ...
  return (
    <Screen scroll={false}>
      <PrimaryButton
        label="Get job updates"
        onPress={() =>
          void requestPushPermission().then((granted) => {
            if (granted) void registerPush(apiClient, 'technician');
            else setHint(true);
          })
        }
      />
      ...
      <FlashList
        ...
        style={styles.list}
        ListEmptyComponent={
          query.isSuccess ? <Text style={styles.empty}>No updates yet</Text> : null
        }
